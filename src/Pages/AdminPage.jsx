...
const AdminPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchTeachers = async () => {
      if (!role) {
        console.log('No role found, skipping teacher fetch');
        return;
      }
      ...
      try {
        // Create collection reference with explicit error handling
        const teacherCollection = collection(db, 'Teacher');
        console.log('Teacher collection reference:', teacherCollection);
        console.log('Collection path:', teacherCollection.path);
        ...
        if (snapshot.empty) {
          console.warn("Teacher collection is empty.");
          setTeachers([]);
        ...
        const data = snapshot.docs.map(doc => {
          ...
          console.log('Teacher doc:', docData);
          return docData;
        });
        
        setTeachers(data);
        console.log('Teachers loaded successfully:', data.length);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Validate profile picture for new teachers
      ...
      try {
        const teacherCollection = collection(db, 'Teacher');
        console.log('Collection reference for save:', teacherCollection);
        
        let docRef;
        if (editingId) {
          console.log('Updating existing teacher:', editingId);
          docRef = doc(db, 'Teacher', editingId);
          ...
          docRef = await addDoc(teacherCollection, teacherData);
      ...
      // Secondary operations (don't let these fail the main operation)
      try {
        console.log('Saving to dawood database...');
        const authToken = await auth.currentUser.getIdToken();
        await addTeacher(teacherData, authToken);
        console.log('Saved to dawood database successfully');
      } catch (dawoodError) {
        console.error('Dawood database error (non-critical):', dawoodError);
      }
      ...
      try {
        ...
        console.error('Mail error (non-critical):', mailError);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      console.log('Deleting teacher:', deleteDialog.id);
